...
### Как убедиться, что настройка даёт результат

Проверка доступности легко обманывает. Ниже — наблюдения, собранные при настройке
пакета на домашнем маршрутизаторе (Netcraze NC-1812).

1. **Критерий истины — браузер, а не `curl`.** Они дают противоположные результаты:
   один и тот же сайт открывался в браузере при обрыве у `curl`, и наоборот — `curl`
   получал `200` там, где браузер рисовал белую страницу. Причина простая: `curl` тянет
   HTML одного адреса, а браузер идёт на десятки хостов статики, и TLS-стек у них разный.
2. **Смотреть объём скачанного, а не факт TLS-рукопожатия.** Рукопожатие может пройти,
   а поток встать сразу после него. «Handshake OK» не значит «страница откроется».
3. **Минимум три прогона на цель.** Результаты плавающие: у меня одна и та же цель за
   15 минут меняла статус четыре раза. По одному прогону вывод делать нельзя.
4. **Один успешный запрос — ещё не работающая страница.** Страница это 20+ запросов
   к разным хостам. Проверять надо набором целей, а не одним `GET`.
5. **Не сбрасывать со счетов DNS-кэш.** Адрес мог смениться (у CDN A-записи ротируются),
   а система, браузер и DNS-прокси на роутере держат старый. Расхождение вида «проба
   идёт на один адрес, браузер на другой» ловится именно так.

#### Широкие диапазоны в `ipset.list`

Если наполняете `ipset.list` самостоятельно, не кладите туда целые `/13` и подобные
диапазоны CDN. Замер на моей сети: хосты внутри `104.16.0.0/13` и `172.64.0.0/13` дали
**70,8 % неудачных TLS-рукопожатий (34 из 48 прогонов)**, хосты вне этих диапазонов —
**0 % (0 из 69)**. При этом `162.158.0.0/15` не давал сбоев ни разу (9 из 9). Похоже на
[bol-van/zapret#1756](https://github.com/bol-van/zapret/issues/1756): при сбое пересборки
TLS оригинальный пакет уходит раньше подменённого, и вместо ожидаемого результата
получается таймаут рукопожатия.

Лечится добавлением проблемного диапазона в `ipset_exclude.list`. Править сам `ipset.list`
смысла нет — при следующем обновлении списка правка потеряется.

### Шифрование имени хоста (ECH)

Если домен публикует DNS-запись HTTPS (RR тип 65) с параметром `ech`, клиент шифрует имя
хоста в ClientHello. `nfqws2` в этом случае имя не видит и не может сопоставить соединение
со списками: профили, привязанные к `user.list`, `exclude.list` и другим хостлистам, для
такого домена не выбираются, и правки этих файлов на него не влияют.

Проверить домен можно так:

```sh
opkg install bind-dig   # Entware и OpenWRT до 24.10 включительно
apk add bind-dig        # OpenWRT 25.xx и Snapshot

dig +short HTTPS example.com
```

Если в выводе есть `ech=`, имя хоста шифруется. Ставить пакет на роутер не обязательно:
запрос можно выполнить с любой машины, где есть `dig`.

Записи различаются даже у соседних доменов одного сервиса — проверено 16.09.2026:

| Домен | HTTPS RR | `ech` |
| --- | --- | --- |
| `discord.com` | есть | нет |
| `discordapp.com` | есть | есть |

Отсюда обычная картина «в браузере страница открывается, а приложение работает иначе»:
браузер обращается к домену без `ech`, а приложение на Electron — к соседнему, где имя
шифруется. Для таких доменов настройка по имени неприменима; остаётся работа по адресам
(`ipset.list`) либо отключение ECH на стороне клиента, если он это позволяет.
